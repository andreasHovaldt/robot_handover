...
while True:
    for color_img, depth_img in zip(color_array,depth_array):
        ...
        if only_human[0]:
            ...
            rescale_human = cv2.resize(only_human8bit,(100,100))
            keypoints, descriptors = sift.detectAndCompute(only_human8bit, None)
            sift_image = cv2.drawKeypoints(only_human8bit, keypoints, only_human8bit)
            #keypoints, descriptors = sift.detectAndCompute(rescale_human, None)
            #sift_image = cv2.drawKeypoints(rescale_human, keypoints, rescale_human)
            cv2.imshow("sift image", cv2.resize(sift_image,(500,500)))
            print(f"descriptors \n {descriptors}")
